...
static void factor_numeric(uint n, const uint *Arp, const uint *Aj,
                           const double *A, struct sparse_cholesky *out,
                           uint *visit, double *y) {
...
void sparse_cholesky_solve(double *x, const struct sparse_cholesky *fac,
                           double *b) {
...
void sparse_cholesky_factor(uint n, const uint *Arp, const uint *Aj,
                            const double *A, struct sparse_cholesky *out,
                            buffer *buf) {
  const uint n_uints_as_dbls =
      (n * sizeof(uint) + sizeof(double) - 1) / sizeof(double);
  ...
  factor_numeric(n, Arp, Aj, A, out, (uint *)buf->ptr,
                 n_uints_as_dbls + (double *)buf->ptr);
}
...
static void csr_mat_dump(const char *name, struct csr_mat *A,
                         const struct comm *c) {
  sint n = A->n, wrk;
  comm_allreduce(c, gs_int, gs_max, &n, 1, &wrk);

  sint rank = (n == A->n) ? c->id : c->np;
  ...
  if (c->id == rank) {
    uint n = A->n, nnz = A->Arp[n];
    fprintf(fp, "%u 1\n", nnz);
    for (uint i = 0; i < n; i++) {
      for (uint j = A->Arp[i], je = A->Arp[i + 1]; j < je; j++)
        fprintf(fp, "%u %u %.15lf\n", i + 1, A->Aj[j] + 1, A->A[j]);
    }
    fclose(fp);
  }
...
static void apply_QQt(struct xxt *data, double *v, uint n, uint tag) {
  ...
  for (;;) {
    ...
    if (other < 0) {
      ...
      comm_isend(&data->req[nsend++], &data->comm, p, size * sizeof(double),
                 other, tag);
    }
    if (lvl == 0)
      break;
...
/* vl += A_ls * vs */
static void apply_p_Als(double *vl, struct xxt *data, const double *vs,
                        uint ns) {
...
/* vs -= A_sl * vl */
static void apply_m_Asl(double *vs, uint ns, struct xxt *data,
                        const double *vl) {
  const uint *Arp = data->A_sl.Arp, *Aj = data->A_sl.Aj;
  const double *A = data->A_sl.A;
  uint i, p, pe;
...
static void apply_S_col(double *vs, struct xxt *data, struct csr_mat *A_ss,
                        uint ei, double *vl) {
...
static struct xxt *crs_setup(uint n, const ulong *id, uint nz, const uint *Ai,
