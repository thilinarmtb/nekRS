                             ...
                             occa::memory o_b)
{
  ...
  if(xn>0 && (!data->null_space || xn>1)) {
    ...
    xxtd->zero(data->sn, data->ln, xxtd->o_vc);
    xxtd->apply_X(xn, data->ln, xxtd->o_vx, xxtd->o_X, xxtd->o_Xp, xxtd->o_vc);
    xxtd->o_vc.copyTo(vc, data->cn * sizeof(dfloat), 0);
#endif
...
//=============================================================================
// XXT interface to nekRS and port to GPUs using OCCA
//
struct xxt_device {
  struct xxt *crs;
  ...
  // Execution buffers
  occa::memory o_vl, o_vc, o_vx, o_combuf;

  // Kernels
  occa::kernel u2c, c2u, apply_m_Asl, apply_p_Als, apply_Xt, zero, apply_X;
};

static int sparse_cholesky_find_level(int **lvl_, uint nr, uint *row_off,
                                      uint *col_id) {
  int *lvl = *lvl_ = tcalloc(int, nr);
  ...
  do {
    ...
    for (i = 0; i < nr; i++) {
      ...
      for (j = row_off[i]; j < row_off[i + 1]; j++)
        if (col_id[j] < i + 1 && lvl[col_id[j] - 1] >= depth)
          depth = lvl[col_id[j] - 1] + 1;
      if (lvl[i] != depth) {
        lvl[i] = depth;
        done = 0;
      }
...
static void build_kernels(struct xxt_device *xxtd) {
  ...
  xxtd->zero = platform->device.buildKernel(file_name, "zero", kinfo);
  xxtd->apply_X = platform->device.buildKernel(file_name, "apply_X", kinfo);
}

static struct xxt_device solver;
static double *h_x = NULL, *h_b = NULL;

int xxt_setup(parAlmond::solver_t* M, uint n, const ulong *id, uint nnz,
  ...
  // Setup XXT on host
  solver.crs = crs_setup(n, id, nnz, Ai, Aj, A, null, &c);
  if (verbose)
    crs_stats(solver.crs);

  // Temps -- get rid of this
  h_x = (double *)tcalloc(double, solver.crs->un);
  h_b = (double *)tcalloc(double, solver.crs->un);

  // Setup parAlmond coarse solver
  M->coarseLevel = new parAlmond::coarseSolver(M->options, M->comm);
  M->coarseLevel->N = solver.crs->un;
  M->coarseLevel->xLocal = M->coarseLevel->rhsLocal = NULL;
  M->baseLevel = M->numLevels;
  M->numLevels++;

  // Setup XXT on device
  allocate_u2c_ids_offsets(&solver);
  allocate_device_arrays(&solver);
  build_kernels(&solver);

  comm_barrier(&c);
  if (c.id == 0 && verbose)
    printf("XXT: Setup done in %g s un = %d\n", comm_time() - start, solver.crs->un);
  fflush(stdout);
...
int xxt_solve(occa::memory o_x, occa::memory o_rhs) {
  struct xxt *xxt = solver.crs;
  o_rhs.copyTo(h_b, xxt->un * sizeof(dfloat), 0);
  crs_solve(h_x, xxt, h_b);
  o_x.copyFrom(h_x, xxt->un * sizeof(dfloat), 0);

  // crs_solve_device(o_x, &solver, o_rhs);
  return 0;
}
