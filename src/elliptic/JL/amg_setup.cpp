...
int nekamg_setup(const char *session, int coars_strat, int interp_strat,
    ...
    double *Aid = (double *)malloc((n-1) * sizeof(double));
    double *Ajd = (double *)malloc((n-1) * sizeof(double));
    double *Av  = (double *)malloc((n-1) * sizeof(double));
    ...
    int *Ai = (int *)malloc((n-1) * sizeof (int));
    int *Aj = (int *)malloc((n-1) * sizeof (int));
    ...
    /* Look for zeros rows */
    int *isrownonzero = (int *)malloc((iuppermax+1) * sizeof(int));
    ...
    struct amg_setup_data *data = (struct amg_setup_data *)malloc(sizeof (struct amg_setup_data));

    /* Initialize local to global correspondence for id */
    data->id_l2g = (int *)malloc((iuppermax+1) * sizeof(int));
    HYPRE_BigInt *id_g2l = (HYPRE_BigInt *)malloc((iuppermax+1) * sizeof(HYPRE_BigInt));
    int c= 0;
    ...
    /* Set local ids */
    data->idl = (int *)malloc((iupper+1) * sizeof(int));
	...
	/* Init matrix */
    HYPRE_IJMatrixCreate(serial, ilower, iupper, ilower, iupper,
                         &ij_matrix);
    ...
    HYPRE_Int **CF_marker_array        = (amg_data)->CF_marker_array;
    ...
    data->n     = (double *)malloc( numlvls    * sizeof (double));
    data->nnz   = (double *)malloc( numlvls    * sizeof (double));
    data->nnzf  = (double *)malloc((numlvls-1) * sizeof (double));
    data->nnzfp = (double *)malloc((numlvls-1) * sizeof (double));
    data->m     = (double *)malloc((numlvls-1) * sizeof (double));
    data->rho   = (double *)malloc((numlvls-1) * sizeof (double));
    data->idc   = (int **)malloc( numlvls    * sizeof (int*));
    data->idf   = (int **)malloc( numlvls    * sizeof (int*));
    data->D     = (double **)malloc((numlvls-1) * sizeof (double*));
    data->Af    = (hypre_CSRMatrix **)malloc((numlvls-1) * sizeof (hypre_CSRMatrix *));
    data->W     = (hypre_CSRMatrix **)malloc((numlvls-1) * sizeof (hypre_CSRMatrix *));
    data->AfP   = (hypre_CSRMatrix **)malloc((numlvls-1) * sizeof (hypre_CSRMatrix *));
    ...
    for (lvl=0;lvl<numlvls;lvl++)
    {
        ...
        if (lvl < numlvls-1)
        {
            ...
            HYPRE_Int *CFlvl = CF_marker_array[lvl]; // CFlvl[i]==1 if i is coarse
            ...
            /* W = P(F,:) */
            int *monec = (int *)malloc(nC * sizeof (int));
            int k;
            ...
            data->idf[lvl] = (int *)malloc(nF * sizeof (int));
            data->idc[lvl] = (int *)malloc(nC * sizeof (int));
            int *idl;
            if (lvl == 0) idl = data->idl;
            else          idl = data->idc[lvl-1];

            /* Save ids for coarse and fine variables */
            HYPRE_Int *indicesF = (HYPRE_Int *)malloc((nF+1) * sizeof (HYPRE_Int));
            indicesF[0] = nF;
            int j, cc=0, cf=0;
            ...
            hypre_ParCSRMatrix **submatricesAf =
                    (hypre_ParCSRMatrix **)malloc(4*sizeof(hypre_ParCSRMatrix*));
...
/* Smoother ----------------------------------------------------------------- */
            ...
            double *D = (double *)malloc(nF * sizeof (double)); // D = diag(Af)' .* s
            diag(D, diagAff);
            vv_op(D, s, nF, ewmult);
            free(s);
            printf("Done.\n");
            ...
            if (nF >= 2)
            {
                ...
                double *Dh = (double *)malloc(nF * sizeof (double)); // Dh = sqrt(D)
                ...
                int k = lanczos(&lambda, diagDhAfDh, serial); // k=number of eigenvalues
                ...
                data->D[lvl] = (double *)malloc(nF * sizeof (double));
...
static void sub_mat(hypre_CSRMatrix **subA, const hypre_CSRMatrix *A,
    const HYPRE_Int *idr, const int *idc)
{
    int rn = A->num_rows, cn = A->num_cols;
    HYPRE_Int *row_off = A->i, *col = A->j;
    double *a = A->data;
    ...
    int c = 0; // counter
    int *g2lcol = (int *)malloc(cn * sizeof (int)); // correspondence between local and
                                             // global column ids
    for (i=0; i<cn; i++)
    {
        if (idc[i] < 0)
    ...
    *subA = hypre_CSRMatrixCreate(subrn, subcn, subncol);
    ...
    HYPRE_Int *subrow_off = subA_ptr->i;
    HYPRE_Int *subcol     = subA_ptr->j;
...
static void amg_export(const struct amg_setup_data *data,char *session)
{
    ...
    double *dvec = (double *)malloc(n * sizeof (double));
    for (i=0;i<nlevels-1;i++)
    {
    ...
    int **id, const int *id_l2g, hypre_CSRMatrix **mat, const char *filename)
{
    ...
    buf = (double *)malloc(2*max * sizeof (double));
    row = (int *)malloc(nl    * sizeof (int));
...
static void savevec(const int nl, const struct amg_setup_data *data,
    ...
    const int *Aff_len, const double *dvec, const char *filename)
{
    int ntot = 2+2*nl+6*n;
    double *q = (double *)malloc(ntot * sizeof (double));
    const double magic = 3.14159;
...
*/
static int lanczos(double **lambda, hypre_CSRMatrix *A, MPI_Comm serial)
{
    int rn = A->num_rows;
    int cn = A->num_cols;
    double *r = (double *)malloc(rn * sizeof (double));
    ...
    *lambda = (double *)malloc(kmax * sizeof (double));
    ...
    double *d = (double *)malloc((kmax+1) * sizeof (double));
    double *v = (double *)malloc(kmax * sizeof (double));
    ...
    double *eye = (double *)malloc(rn * sizeof (double));
    init_array(eye, rn, 1.);

    /* Frobenius norm */
    hypre_CSRMatrix *Ae;
    dpm(&Ae, A, eye, dsub, serial);
    ...
    double *qk = (double *)malloc(rn * sizeof (double));
    init_array(qk, cn, 0.);
    double *qkm1 = (double *)malloc(rn * sizeof (double));
    double *alphaqk = (double *)malloc(rn * sizeof (double)); // alpha * qk vector
    double *Aqk = (double *)malloc(rn * sizeof (double));
    int na = 0, nb = 0;
...
static void dpm(hypre_CSRMatrix **AD, hypre_CSRMatrix *A, const double *D,
    enum dpm_ops op, MPI_Comm serial)
{
    int rn = A->num_rows;
    ...
    HYPRE_IJMatrix D_ij;
	/* Init matrix */
    HYPRE_IJMatrixCreate(serial, 0, rn-1, 0, rn-1, &D_ij);
