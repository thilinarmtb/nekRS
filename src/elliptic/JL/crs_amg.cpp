...
static void crs_solve(double *x, struct crs_data *data, double *b)
{
                         ...
                         struct gs_data *gs_top, buffer *const buf)
{
  ...
  { ulong *const uid_p = (ulong *)uid->ptr;
...
static void amg_setup_mat(
  ...
  buffer *buf)
{
  slong *id = (slong *)ids->ptr; ulong last_id = ULONG_MAX;
  const uint ne = ids->n;
  uint k,i=0,ie=nloc;
  ...
  for(k=0;k<nz;++k) {
    ...
    id = (slong *)array_reserve(slong, ids, ids->n+1);
  ...
  buffer *buf)
{
  ...
  for(m=0;m<3;++m) {
    ...
    localize_rows((struct gnz *)mat[m].ptr,mat[m].n, uid,id_perm);
    /* sort by row */
    sarray_sort(struct gnz,mat[m].ptr,mat[m].n, i,1, buf);
    /* find offsets of each level */
    find_mat_offs(mat_off[m],levels,(const struct gnz *)mat[m].ptr,mat[m].n, id);
  }
  /* allocate CSR arrays */
  if(levels>1) {
  ...
  for(lvl=0;lvl<levels-1;++lvl) {
    ...
    slong *p = (slong *)array_reserve(slong, &ide, nloc);
    ...
    data->Q_W[lvl].gsh = gs_setup((const slong *)ide.ptr,ide.n, &data->comm, 0,gs_auto,1);
    amg_setup_mat(&ide,nloc, &data->AfP[lvl], 
                  uid, uid_n,  id_perm, off[lvl],j0,
                  (struct gnz*)mat[1].ptr + mat_off[1][lvl],
                  mat_off[1][lvl+1]-mat_off[1][lvl], buf);
    data->Q_AfP[lvl].gsh = gs_setup((const slong *)ide.ptr,ide.n, &data->comm, 0,gs_auto,1);
  ...
  for(lvl=0;lvl<levels-1;++lvl) {
    ...
    data->Q_Aff[lvl].gsh = gs_setup((const slong *)ide.ptr,ide.n, &data->comm, 0,gs_auto,1);
    if(ide.n>max_e) max_e=ide.n;
  }
  free(mat_off[0]);
  array_free(&ide);
...
static void amg_setup_aux(struct crs_data *data,  uint n, const ulong *id, const char *datafname)
{
  ...
  sortp_long(&cr.data,0,(const ulong *)uid.ptr,uid.n,sizeof(ulong));
  sarray_permute(ulong,(uint *)uid.ptr   ,uid.n, (uint *)cr.data.ptr, &temp_long);
  sarray_permute(uint ,data->umap,uid.n, (uint *)cr.data.ptr, &max_e);

#ifdef USEMPIIO
  read_data_mpiio(data, &ids, mat, &cr, uid.ptr,uid.n, datafname);
#else
  read_data(data, &ids, mat, &cr, (const ulong *)uid.ptr,uid.n, datafname);
#endif
  ...
  sarray_permute(uint,data->umap,ids.n, (uint *)cr.data.ptr, &max_e);
  id_perm = tmalloc(uint, uid.n);
  sarray_perm_invert(id_perm, (const uint *)cr.data.ptr, uid.n);
  /* the global id   uid[i]   will have local index    id_perm[i]
     (the local storage is sorted by level) */

  data->lvl_offset = compute_offsets((const struct id_data *)ids.ptr,ids.n, data->levels);

  max_e = amg_setup_mats(data,(const ulong *)uid.ptr,uid.n,id_perm,
                         (const struct id_data*)ids.ptr,mat, &cr.data);

  { const unsigned levels=data->levels;
    const uint *const off = data->lvl_offset;
    struct id_data *const id = (struct id_data *)ids.ptr; const uint n = ids.n;
    double *d;
    uint i,max_f=0;
...
static struct crs_data *crs_setup(
  uint n, const ulong *id,
  uint nz, const uint *Ai, const uint *Aj, const double *A,
...
static void crs_free(struct crs_data *data)
{
  const unsigned levels = data->levels;
  ...
  struct crystal *const cr)
{
  ...
  data->map.n=n;
  for(q=(struct find_id_map *)data->map.ptr,i=0;i<n;++i)
  ...
  const ulong *id, const unsigned id_stride, const uint n)
{
  ...
  const struct find_id_map *q=(const struct find_id_map *)data->map.ptr,
    *const q_end = (const struct find_id_map *)(q+data->map.n);
  const uint np = data->cr->comm.np;
  int not_found = 0;

  uint nn;
  p = (struct find_id_work *)array_reserve(struct find_id_work, &data->work, n);
  ...
  sarray_sort(struct find_id_work,data->work.ptr,data->work.n, id,1,
    &data->cr->data);
  for(p=(struct find_id_work *)data->work.ptr,p_end=p+data->work.n;p!=p_end;++p) {
  ...
  sarray_sort(struct find_id_work,data->work.ptr,data->work.n, id,1,
    &data->cr->data);
  p = (struct find_id_work *)data->work.ptr;
  ...
  const ulong *uid, const uint uid_n, const char *datafname)
{
  ...
  for(r=0;r<tn;r+=AMG_BLOCK_ROWS) {
    ...
    if(pid==0) {
      unsigned i;
      struct id_data *idp = (struct id_data *)id_buffer.ptr;
      double *b = (double *)read_buffer.ptr;
      dread(b,nr*6, f,&code);
      if(code==0) {
      ...
      /* sort id_buffer, remember how to undo */
        ...
        sarray_perm_invert(id_perm, (const uint *)cr->data.ptr, nr);
      }
    } else
      id_buffer.n=0;
    ...
    for(m=0;m<3;++m) {
      if(pid==0) {
        const struct id_data *const idp = (const struct id_data *)id_buffer.ptr;
        unsigned i; uint j,rl;
        struct gnz *p = (struct gnz *)array_reserve(struct gnz, &mat_buffer, mat_size[m]);
        double *b = (double *)array_reserve(double, &read_buffer, 2*mat_size[m]);
        uint *proc = (uint *)array_reserve(uint, &mat_proc, mat_size[m]);
      ...
      comm_bcast(&data->comm,&code,sizeof(int),0);
      if(code!=0)die(1);
      sarray_transfer_ext(struct gnz,&mat_buffer,(const uint *)mat_proc.ptr,sizeof(uint),cr);
...
static void mat_condense_sorted(struct array *const mat)
{
  ...
  if(mat->n<=1) return;
  for(dst=(struct rnz *)mat->ptr;;++dst) {
    if(dst+1==end) return;
  ...
  const ulong *uid, struct crystal *const cr)
{
  ...
  #define BUILD_LIST(k) do { \
    ...
    for(count=0,p=(const struct rnz *)mat->ptr;p!=e;++p) { \
      if(p->k.p==pid || rid_equal(last_rid,p->k)) continue; \
      last_rid=p->k; ++count; \
    } \
    array_init(struct labelled_rid, nonlocal_id, count); \
    nonlocal_id->n=count; out=(struct labelled_rid *)nonlocal_id->ptr; \
    last_rid.p=UINT_MAX,last_rid.i=UINT_MAX; \
    for(p=(const struct rnz *)mat->ptr;p!=e;++p) { \
      if(p->k.p==pid || rid_equal(last_rid,p->k)) continue; \
  ...
  for(out=(struct labelled_rid *)nonlocal_id->ptr,end=out+nonlocal_id->n;out!=end;++out)
  ...
  struct crystal *const cr)
{
  ...
  vi=(double *)buf->ptr, vj=vi+n, va=vj+n;
  rlbl = (const struct labelled_rid *)nonlocal_id.ptr;
  for(nz=(const struct rnz *)mat->ptr,enz=nz+n;nz!=enz;++nz) {
    *vi++ = uid[nz->i.i];
    *va++ = nz->v;
    if(nz->j.p==pid)
      *vj++ = uid[nz->j.i];
  ...
  struct crs_data *data, const char *datafname)
{
  ...
  for(out=(struct rnz *)mat.ptr,k=0;k<nz;++k) {
  ...
  dump_matrix(&mat,(const ulong *)uid.ptr,&cr,datafname);
...
//=============================================================================
// AMG interface to nekRS and port to GPUs using OCCA
//
struct crs_data *solver;
static double *h_x = NULL, *h_b = NULL;
static uint un;

int amg_setup(parAlmond::solver_t* M, uint n, const ulong *id, uint nnz,
  ...
  uint err;
  solver = crs_setup(n, id, nnz, Ai, Aj, A, null, &c, "crs_amg_data", &err);
  if (solver == NULL) {
    comm_free(&c);
    return 1;
  }
  ...
  un = n;
  h_x = (double *)tcalloc(double, n);
  h_b = (double *)tcalloc(double, n);

  comm_barrier(&c);
  if (c.id == 0 && verbose)
    printf("AMG: Setup done in %g s un = %d\n", comm_time() - start, solver->un);
  fflush(stdout);
...
int amg_solve(occa::memory o_x, occa::memory o_rhs) {

  o_rhs.copyTo(h_b, un * sizeof(dfloat), 0);
  crs_solve(h_x, solver, h_b);
  o_x.copyFrom(h_x, un * sizeof(dfloat), 0);
